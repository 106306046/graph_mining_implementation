...
if __name__ == "__main__":
    ...
    hid_size = [16, 32, 64, 128, 256, 512]
    head_list = [2, 3, 4, 5]

    best_acc = 0
    best_model = None

    for hid_size_i in hid_size:
        for head in head_list:
            # model = GCN(in_size, hid_size_i, out_size).to(device)
            model = YourGNNModel(in_size, hid_size_i, out_size, head).to(device)
            ...
            print("inter_size:", hid_size_i, "head:", head, "acc:", acc)
