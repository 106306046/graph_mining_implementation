...
class Server:
    def __init__(self, args, model, seed, logging):
        ...
        self.K = args.K
        self.model = copy.deepcopy(model[0]) #深複製：複製對象是完全新的
    ...
    def aggregate_parameters(self):
        ...
        for user in self.selected_users:
            total_weight += user.train_samples #先算 total 數量
            
        for user in self.selected_users:
            for param_user,param_self in zip(user.model.parameters(),self.model.parameters()):
                if first:
                    param_self.data = (user.train_samples/total_weight)*param_user.data #第一次param取代目前的param
                else:
                    param_self.data += (user.train_samples/total_weight)*param_user.data #第二個以後的param加權上去
    ...
    def select_users(self, round, num_users):
        ...
        if num_users > len(self.users):
            print('num_users > len(self.users)')
            return self.users #如果不夠長就直接 return 所有的 user
        else:
            random.seed(round) #用 round 當 seed 比較好比較
            select_user_list = random.choices(self.users,k=num_users) #從目前 user list 去找 selected user
