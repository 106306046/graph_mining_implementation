...
class Judge:
    ...
    def multi_test(self, func, **kwargs):
        ...
        for target_node in target_nodes:
            n_perturbations = (
                int(degrees[target_node]) + 2
            )  # budget  = degree of target node + 2
            ...
            actural_perturbations = (
                (self.adj.toarray() - modified_adj.toarray()) ** 2
            ).sum() / 2  # symmetric
    ...
    def single_test(
        self, adj: Union[sp.spmatrix, torch.Tensor], target_node: int
    ) -> float:
        check_adj(adj)
